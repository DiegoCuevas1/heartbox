...
export default function MemberList({members = [],invite_code,id}:MemberListProps){
    ...
    return(
        ...
        <>
            <div className="flex justify-center items-center space-x-2 mt-2">
                ...
                <div className="flex">
                {members && members?.length>0 &&<Image src={`https://dev-heartbox.s3.us-east-2.amazonaws.com/profile_pics/${members?.at(0)?.profile_picture}`} alt={""} className="h-12 rounded-full " style={{objectFit:'cover'}} width={50} height={100}/>}
                {members && members?.length>1 &&<Image src={`https://dev-heartbox.s3.us-east-2.amazonaws.com/profile_pics/${members?.at(1)?.profile_picture}`} alt={""} className="h-12 rounded-full " style={{objectFit:'cover'}} width={50} height={100}/>}
                {members && members?.length>2 &&<Image src={`https://dev-heartbox.s3.us-east-2.amazonaws.com/profile_pics/${members?.at(2)?.profile_picture}`} alt={""} className="h-12 rounded-full " style={{objectFit:'cover'}} width={50} height={100}/>}
                </div>
                {members && members.length>0 ?<p className="text-lg">
                <Link href={{
                    pathname: `/families/${id}/members`,
                }}>
                    {members?.length} members</Link></p> : <></>}
