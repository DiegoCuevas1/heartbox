...
function FormComponent() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(sanitize_res_msg(resMsg));
        ...
        setIsLoading(false);
        toast.error(sanitize_res_msg(resMsg));
      }
    } catch (error) {
      setIsLoading(false);
            ...
            <>
              ...
              <div className="flex space-x-2">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="mx-auto w-36 h-12 text-[#d31c60] font-loves font-bold hover:scale-125 transition-all active:scale-95 text-2xl rounded-xl bg-[#ffffff] disabled:opacity-50 disabled:hover:scale-100"
                >
                  {isLoading ? "..." : "Submit"}
                </button>
              </div>
